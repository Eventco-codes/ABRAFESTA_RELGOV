...
import { uploadAnexoPauta, type AnexoFormState } from "../actions";

export function AnexoUploadForm({ pautaId }: { pautaId: string }) {
  const [open, setOpen] = useState(false);
  const formRef = useRef<HTMLFormElement>(null);
  const [state, formAction, pending] = useActionState<AnexoFormState, FormData>(
    async (prev, formData) => {
      const result = await uploadAnexoPauta(pautaId, prev, formData);
      if (!result.error) {
